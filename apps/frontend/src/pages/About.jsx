      ...
      <main className="flex-grow">
          ...
          <div className="max-w-[1440px] mx-auto relative z-10">
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <p className="text-lg md:text-xl text-blue-100/70 max-w-3xl leading-relaxed text-pretty">
                DermaVisio adalah platform berbasis Artificial Intelligence yang dirancang untuk membantu deteksi dini lesi kulit melalui analisis citra dermatoskopi menggunakan teknologi Convolutional Neural Network (CNN).
              </p>
            </div>
          </div>
        ...
        <section className="max-w-[1440px] mx-auto px-6 lg:px-20 py-24">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-4">Klasifikasi Lesi Kulit</h2>
            <p className="text-slate-500">Sistem kami mampu mengidentifikasi 7 kategori kondisi kulit utama.</p>
          </div>

          <div className="grid grid-cols-1 gap-4 max-w-5xl mx-auto">
            {skinLesions.map((lesion, idx) => (
              <div key={idx} className="group flex flex-col md:flex-row items-center gap-6 bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-500">
                {/* Image Box */}
                <div className="w-full md:w-24 h-40 md:h-24 shrink-0 rounded-2xl overflow-hidden border border-slate-100 bg-slate-50">
                  <img src={lesion.image} alt={lesion.name} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                </div>
                {/* Content Box */}
                <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="text-sm text-slate-500 leading-relaxed">{lesion.desc}</p>
                </div>
        ...
        <section className="max-w-[1440px] mx-auto px-6 lg:px-20 py-24">
          <h2 className="text-3xl md:text-5xl font-black text-slate-900 text-center mb-20 tracking-tight">
            Di Balik <span className="text-blue-600">DermaVisio</span>
          </h2>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {teamMembers.map((member, index) => (
              <div key={index} className="group relative bg-white rounded-[2.5rem] border border-slate-100 p-6 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col items-center">
                ...
                <p className="text-blue-600 font-medium text-sm mb-6">{member.role}</p>
        ...
        <section className="max-w-[1440px] mx-auto px-6 lg:px-20 pb-24">
           <div className="bg-red-50 border-2 border-red-100 rounded-[3rem] p-10 md:p-16 text-center space-y-4">
              ...
              <h2 className="text-red-900 text-3xl font-black tracking-tight">Peringatan Medis Penting</h2>
              <p className="text-red-700/80 max-w-4xl mx-auto text-lg leading-relaxed font-medium">
                DermaVisio adalah alat bantu skrining awal dan <strong>bukan pengganti diagnosa medis profesional</strong>. Seluruh hasil analisis harus dikonsultasikan kembali dengan dokter spesialis kulit berlisensi sebelum mengambil tindakan medis apa pun.
              </p>
           </div>
        </section>
      </main>
